...
19. **最上級・比較優位・全称表現は使わない。** 「最も厳しい」「No.1」「100%が〜」「スーパーより鮮度が高い」はNG。仕組みや事実だけ述べる。詳細は `.claude/rules/feedback-log.md` 参照。
20. **ランキング・おすすめ記事には免責注記を必ず入れる。** 導入文末とまとめ末の2箇所にテンプレートを配置する。テンプレートは `.claude/rules/feedback-log.md` FB-H3 参照。
21. **キャンペーン・割引・返金保証は条件付き表現で書く。** 固定値（「最大〇〇円」「全額返金」）で断言せず、「内容・条件は公式サイトで要確認」を付記する。
...
| ルール | `.claude/rules/feedback-log.md` | 法令・品質チェックFBの抽象化ルール |
