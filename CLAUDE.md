...
18. **法令セーフ変換表（下記10パターン）を執筆段階で必ず適用する。** gensparkチェック前に全て潰す。公式サイトに記載されている表現でも、記事側では法令セーフ表現に変換する。
19. **過去FBの再発防止ルール（`.claude/rules/feedback-log.md`）を執筆前に必ず読む。** 割引表記の比較対象明示（FL-1）、制限食表現の置換（FL-2）、書き出しパターン3種化（FL-4）、H2直下画像配置（FL-5）、根拠なし品質訴求の禁止（FL-9）は特に頻発するため要注意。
20. **H2②サービス紹介の冒頭には横断比較テーブル（サービス名・価格・送料の3列のみ）を配置する。** 栄養素の列は薬機法リスクがあるため絶対に入れない。各H3にid属性を付与してジャンプリンクにする（FL-11）。
21. **1文50文字以内（目安30文字）。PREP構造を全H3で徹底。** 1文目は必ず結論。説明・リスク・理由から始めない。同じワードをH3内で3回使わない（FL-18,19,20）。
22. **「防げます」「注意が必要です」「直結」「設計です」「強みです」は使用禁止。** lint.shで検出されないNG語もあるため、FL-25のリストを毎回確認する。
