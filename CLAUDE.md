...
15. **画像URLはservice-info.mdからコピペする。自分で推測・記憶から書かない。** wp-image-XXXの番号、uploads/YYYY/MM/のパス、ファイル名、width/heightの全てをservice-info.mdの値と完全一致させる。1文字でも違えばWordPress上で画像が表示されない。
...
17. **Manabuさんから構成（H2/H3見出し一覧）が直接指定された場合は、1文字も変えずにそのまま採用する。** researcherが独自に構成案を再設計しない。指定構成に基づいて申し送り事項のみ作成する。
...
```
⚠️ 出力分割: researcherは1回でステップ1〜7を全部出さない。ステップごとに分割して出力する。
